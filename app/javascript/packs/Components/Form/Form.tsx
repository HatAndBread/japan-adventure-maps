...
const Form = ({
  ...
  const controllerAction: string = useAppContext().controllerData.controllerAction;
  ...
  return (
    <form
      ...
      encType={hasFile ? 'multipart/form-data' : 'application/x-www-form-urlencoded'}
      method={getMethod()}
      acceptCharset='UTF-8'
      action={action}
      id={id ? id : ''}
      ...
      ref={formRef}>
      {lc === 'patch' || lc === 'put' ? <input type='hidden' name='_method' value={lc} /> : <></>}
      <AuthenticityToken />
      {children}
    </form>
  );
